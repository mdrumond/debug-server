...
2. Download and install Miniconda when `conda` is not already on `PATH`, then create or update the Conda environment declared in `environment.yml` (or create `.venv` when configured).
