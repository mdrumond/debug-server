...
- **Bootstrap-first workflow:** A dedicated bootstrap task provisions the Conda (or virtualenv) runtime, verifies the already cloned repository, and captures secrets/configuration in `config/bootstrap.toml`. Every other task assumes this reproducible foundation and reuses the smoke-test command described in [`.codex/tasks/bootstrap-environment-and-repo-sync.md`](tasks/bootstrap-environment-and-repo-sync.md).
- **Bootstrap artifacts now implemented:** `scripts/bootstrap.py` drives the automation, `config/bootstrap.toml` stores host-specific overrides, `environment.yml` defines the Conda environment, and `tests/bootstrap/` contains smoke tests that validate repo + storage setup. Operators must run `./scripts/bootstrap.py --check` before provisioning hosts to ensure prerequisites (`git`, Python 3.11+) are available. When `conda` is missing the bootstrap script downloads Miniconda (accepting the license with the installer `-b` flag) into `.artifacts/miniconda3`, but it can still fall back to `.venv` creation and pip-install the dependencies declared in `environment.yml` when `allow_venv_fallback` is true in the config file.
