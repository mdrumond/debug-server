...
- **Bootstrap-first workflow:** A dedicated bootstrap task provisions the Conda runtime, verifies the already cloned repository, and captures secrets/configuration in `config/bootstrap.toml`. Every other task assumes this reproducible foundation and reuses the smoke-test command described in [`.codex/tasks/bootstrap-environment-and-repo-sync.md`](tasks/bootstrap-environment-and-repo-sync.md).
- **Bootstrap artifacts now implemented:** `scripts/bootstrap.py` drives the automation, `config/bootstrap.toml` stores host-specific overrides, `environment.yml` defines the Conda environment, and `tests/bootstrap/` contains smoke tests that validate repo + storage setup. Operators must run `./scripts/bootstrap.py --check` before provisioning hosts to ensure prerequisites (`git`, Python 3.11+) are available. When `conda` is missing the bootstrap script downloads, verifies, and installs the Linux x86_64 Miniconda build into `.artifacts/miniconda3`. Non-Linux hosts must point `conda_installer_url` and `conda_installer_sha256` at an appropriate installer before running the script.
- **SQLite + SQLModel metadata core:** The execution service persists repositories, worktrees, sessions, commands, artifacts, and auth tokens in SQLite via SQLModel. Alembic migrations keep the schema consistent and the data layer exposes transactional helpers for higher layers (see [`.codex/tasks/server-backend-sqlite-metadata-store.md`](tasks/server-backend-sqlite-metadata-store.md)).
  - Implementation lives under `debug_server/db/` and includes Alembic migrations (`python -m debug_server.db.migrate upgrade head`),
    an admin CLI for creating tokens (`python -m debug_server.db.admin create-token <name>`), and test helpers inside
    `debug_server/db/testing.py`. See [`docs/architecture.md`](../docs/architecture.md) for the schema overview.
- **Worktree + dependency pool:** Implemented in [`debug_server/worktrees/`](../debug_server/worktrees/) with documentation in [`docs/worktrees.md`](../docs/worktrees.md). `WorktreePool` keeps a bare mirror in `.artifacts/repos/`, clones per-session checkouts, records leases in SQLite, and exposes `reclaim_stale_worktrees()` plus Typer CLIs (`python -m debug_server.worktrees.inspect ...`). Dependency fingerprints are tracked via `DependencyStateStore`, allowing runners to skip reinstalling environments until lockfiles change (see [`.codex/tasks/server-backend-worktree-and-session-cache.md`](tasks/server-backend-worktree-and-session-cache.md)).
- **Runner/worker separation:** The runner supervisor prepares environments, applies patches, executes commands, and streams logs. Debugger orchestration builds on top of the same supervisor to expose debugpy/gdb/lldb tunnels. These capabilities are captured in [`.codex/tasks/server-runner-worker-engine.md`](tasks/server-runner-worker-engine.md) and [`.codex/tasks/server-runner-debugger-integration.md`](tasks/server-runner-debugger-integration.md). The initial worker engine now lives under `debug_server/runner/` with documentation in [`docs/runner.md`](../docs/runner.md) and configuration in [`config/runner.toml`](../config/runner.toml).
- **FastAPI service surface:** FastAPI hosts lifecycle endpoints plus WebSocket routes for logs/debug control. Authentication is handled via HTTP bearer tokens stored in the metadata DB. The separation between standard REST endpoints and streaming/debug routers is tracked via [`.codex/tasks/server-api-lifecycle-and-auth.md`](tasks/server-api-lifecycle-and-auth.md) and [`.codex/tasks/server-api-debug-streams.md`](tasks/server-api-debug-streams.md).
    - Implementation now lives under `debug_server/api/`. `main.py` wires up routers for repository initialization, session lifecycle, command queueing, artifact downloads, and token management. `schemas.py` shares Pydantic models with the CLI/MCP client code, and `auth.py` provides the reusable HTTP/WebSocket bearer validation with scope-aware checks. `middleware.py` emits structured audit logs for every HTTP request so operators can trace activity back to a token.
    - Repository lifecycle endpoints (`/repository/*`) wrap the metadata store helper methods to upsert the upstream configuration and expose read APIs for clients.
    - Session routes (`/sessions/*`) create/cancel sessions, expose metadata, and stream artifacts via `FileResponse`. Command queueing lives under `/sessions/{id}/commands` while `/auth/tokens` lets admins mint/revoke bearer tokens without touching the CLI admin utilities.
