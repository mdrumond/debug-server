...
- **Bootstrap-first workflow:** A dedicated bootstrap task provisions the Conda (or virtualenv) runtime, clones the bare repo mirror, and captures secrets/configuration in `config/bootstrap.toml`. Every other task assumes this reproducible foundation and reuses the smoke-test command described in [`.codex/tasks/bootstrap-environment-and-repo-sync.md`](tasks/bootstrap-environment-and-repo-sync.md).
- **Bootstrap artifacts now implemented:** `scripts/bootstrap.py` drives the automation, `config/bootstrap.toml` stores host-specific overrides, `environment.yml` defines the Conda environment, and `tests/bootstrap/` contains smoke tests that validate repo + storage setup. Operators must run `./scripts/bootstrap.py --check` before provisioning hosts to ensure prerequisites (`git`, `conda`, Python 3.11+) are available. The bootstrap script can fall back to `.venv` creation when Conda is unavailable but `allow_venv_fallback` is true in the config file.
