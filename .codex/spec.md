...
- **Worktree + dependency pool:** A backend module manages git worktrees and dependency caches, leasing them to sessions and enforcing TTL/LRU eviction. The runner relies on this module for quick startup (see [`.codex/tasks/server-backend-worktree-and-session-cache.md`](tasks/server-backend-worktree-and-session-cache.md)).
